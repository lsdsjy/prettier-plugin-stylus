...
export type RGBA = InstanceType<Nodes['RGBA']>;
  export type Node = Omit<InstanceType<Nodes['Node']>, 'nodeName'> &
    (
    | ({ nodeName: 'root'} & Root)| ({ nodeName: 'block'} & Block)| ({ nodeName: 'group'} & Group)| ({ nodeName: 'selector'} & Selector)| ({ nodeName: 'ident'} & Ident)| ({ nodeName: 'property'} & Property)| ({ nodeName: 'expression'} & Expression)| ({ nodeName: 'literal'} & Literal)| ({ nodeName: 'call'} & Call)| ({ nodeName: 'arguments'} & Arguments)| ({ nodeName: 'unit'} & Unit)| ({ nodeName: 'each'} & Each)| ({ nodeName: 'function'} & Function)| ({ nodeName: 'params'} & Params)| ({ nodeName: 'binop'} & BinOp)| ({ nodeName: 'comment'} & Comment)| ({ nodeName: 'string'} & String)| ({ nodeName: 'rgba'} & RGBA)
    )
}
